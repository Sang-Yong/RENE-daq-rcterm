...
NR_=$(grep -c '이번 하드에 담을 것' "$T/out.txt")
...
if grep -q '회차 — 자리가 남아 다시 계획' "$T/out.txt"; then ok "★ 다시 계획했다고 알린다"
else bad "다시 계획하지 않았다" "$(grep -c '이번 하드에 담을 것' "$T/out.txt") 번만 계획"; fi
NR_=$(grep -c '이번 하드에 담을 것' "$T/out.txt")
...
if grep -q '원본 일부가 사라졌습니다' "$T/out.txt"; then ok "사유를 밝힌다"
else bad "안내가 없다" "$(grep -E '❌|⚠️' "$T/out.txt" | head -2)"; fi
if grep -q '살아남은 파일' "$T/out.txt"; then ok "살아남은 개수를 센다"
