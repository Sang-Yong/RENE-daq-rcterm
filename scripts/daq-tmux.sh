...
#
#      +--------------------------+--------------------+
#      | DAQ Run Status(monitor)  |  work space (vi 등)|
#      |   = rcmon.sh          28 |                  7 |
#      +--------------------------+                    |
#      | supervisor             8 |                    |
#      +--------------------------+--------------------+
...
#
#      pane 제목은 사람이 읽으라고 붙이는 것이지만 daq-layout.sh 가 이것으로
#      pane 을 찾는다. 바꿀 때는 그쪽 열쇠말(monitor / supervisor / postrun /
#      work space / dataflow)이 제목 안에 남아 있는지 확인할 것.
#
#  사용 :
