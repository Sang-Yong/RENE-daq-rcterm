...
**★ `/scratch` 에서는 이 작업이 매우 느리다.** 옛 런은 PRD 하나가 681 MB 이고
100 Mb 링크를 dataflow 와 나눠 쓴다 — 서브런 하나에 **6분**이 걸렸다(로컬
`/Data_ssd` 에서는 7초다, §11.32 의 13배 차이와 같은 이유).

**3526:00081 은 실패했다** — `[ERROR] 'AbsEvent' TTree missing in MERGED file`.
Merged 파일이 있어도 온전하다는 뜻은 아니다. 매크로의 방어막이 잡아냈다.
