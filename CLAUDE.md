...
- **run 4288·4289 도 RAW/PRD/PNG 가 미백업이다.** 각 1440 서브런, 합쳐 약
  460 GB 이고 전부 `/scratch` 에 있어 느린 쪽 링크를 탄다. 사용자 확인 대기.
- `/scratch/LOG` 의 postrun merge 로그(`log_merge_*.txt`, 전체 343,733개)를
  백업할지. 원격 `DAQLOG` 는 `FADCDAQ`/`SADCDAQ`/`TCB` 세 종류만 쌓는 구조다.
- **`dataflow.sh` pane 을 한 번 재시작해야 한다.** rename 으로 갈아끼워서
  돌고 있는 프로세스(pid 104510)는 옛 inode 를 계속 읽는다. 새 체크섬 검증이
  실제로 적용되려면 4292 가 끝나 1단계가 움직이기 전에 재시작할 것.
