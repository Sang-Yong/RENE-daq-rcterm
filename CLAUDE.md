...
- **★ 도는 스크립트 위에 덮어쓰지 말 것 — 저장소 서버 배포는 임시 이름 + `mv`.** `scp a store:b` 는 b 를 제자리에서
  덮어써 inode 가 같다. 세션이 그 파일을 붙들고 있으면 루프 밖 최상위 코드가 어긋난 바이트를 읽는다 (§11.185).
  ```
  scp scripts/storage-backup.sh store:/home/frontend/data_backup_simple_code9.sh.new
  ssh store 'chmod +x …code9.sh.new && mv -f …code9.sh.new …code9.sh'     # rename = 새 inode, 도는 세션은 옛 것을 계속 붙든다
  ```
...
#### 11.205 ★ 타겟 뮤온 NPE 스펙트럼을 veto 태그 유무 · 기간별로 (사용자 지시, 09-18 06:2x) — `tools/monitor/MuonSpec.C` · `muon-spec.sh`

사용자 : "4237~4348 데이터로 검출기 안 뮤온 에너지 스펙트럼을 veto 태그 유무에 따라, 런 전체와 월·주·런별로 따로 그려 태깅에 따라 타겟 스펙트럼이
어떻게 바뀌는지 보고 싶다." commit 3b88d97. 산출 `/scratch/RunSummary/muspec/<dst|dst_m2>/` : `muspec_all_all` · `muspec_month_<YYYY-MM>` ·
`muspec_week_<YYYY-Www>` · `muspec_run_<런>` (위 스펙트럼 태그/미태그/합, 아래 빈별 태그 비율) · `muspec_overlay_{months,weeks}` · `muspec_trend`(주별·런별
태그 비율 > 3000 / > 20000 NPE, 문턱 표식) · `muspec_summary.tsv` · `muspec.root`. 64 장 × 2 벌, zip `/Data_ssd/LOG/muspec-20260918.zip`.
정의 : 태그됨 = T_Muons 의 pe > 0(veto 가 걸리고 타겟에도 신호), 미태그 = T_Sat ∪ T_Singles. 둘 다 620 NPE(single 문턱 610.6) 위만. 기간은 서브런 중간 시각(지역시),
라이브타임은 서브런 등분. x = 두 PMT NPE 합(1 µs 적분, ~3.5×10⁴ 에서 포화 상한). 전 런 46 개 · 39.6 라이브 일, 두 벌 각 5 분.
```
                            패널 AND (dst)                              강한 veto (dst_m2)
전체    >3k  태그 59.1 %  (11.7 만/일 대 8.1 만/일)    >20k 71.7 %       >3k 83.7 %   >20k 93.7 %
2026-06 (4237)  >3k 72.0 %  >20k '100 %' ★                               85.3 %  /  94.7 %
2026-08         54.8 %  /  64.2 %                                        84.5 %  /  93.5 %
2026-09         58.1 %  /  69.3 %  (W37 66/79 % = 4344~4346 ch2 100 되돌림 · W38 49/58 % = 4347~ 80 % 문턱)   82.0 %  /  93.4 %
빈별 비율 (dst) : 3,000 NPE 아래 0 → 3~5k 에서 60 % 로 급등 → 1~2×10⁴ 에서 80 % 최대 → 포화 영역(>2.5×10⁴) 에서 55~65 % 로 다시 내려온다
빈별 비율 (dst_m2) : 620~3k 에서도 8~15 % (γ·잡음까지 veto) → 3k 위 75 % → 1~2×10⁴ 96 % → 포화 영역 90 %
```
**★ 사용자가 보려던 그림 (두 번째 지시, 07:2x)** — 위아래 두 패널이 아니라 **한 캔버스에 veto 컷 전(태그 무관 전체) 과 컷 뒤(태그 안 된 것만) 스펙트럼**을
같이 : `muspec_cut_<kind>_<key>[_log].png` · `muspec_cut_overlay_{months,weeks}[_log].png` (실선 = 컷 전, 점선 = 컷 뒤, 색 = 기간). 기본 선형(5,000 NPE 아래
γ 는 위로 잘림), 로그는 `_log`. 사용자는 **선형축을 선호**한다 — 앞으로 기본은 선형, 로그는 따로 (메모리 plot-rules 에 적음). commit b2e0d0b · c7bd0c0.
```
컷 뒤 남는 뮤온 (>3k / >20k NPE, 하루)   패널 AND (dst)                       강한 veto (dst_m2)
2026-06 (4237)     10.2만 (48 %) / 3.5만 (40 %) [재생성 뒤]   3.3만 (15 %) / 4.9천 (5 %)
2026-08            9.5만 (45 %) / 3.0만 (36 %)              3.5만 (16 %) / 5.6천 (6 %)
2026-09            8.9만 (42 %) / 2.7만 (31 %)              3.9만 (18 %) / 6.0천 (7 %)
```
즉 패널 AND 컷은 관통 뮤온 봉우리(2.2×10⁴)를 1/3 로만 줄이고, 강한 veto 는 1/15 로 줄인다. 봉우리 모양은 컷 뒤에도 같다 (새는 것은 에너지에 무관).

**읽는 법.** ① 스펙트럼 모양은 기간에 무관하게 같다 (2×10³ 아래 γ 배경, 5×10³~10⁴ 골, 1.5~2.5×10⁴ 의 관통 뮤온 봉우리, 3.5×10⁴ 포화). 태깅이 바꾸는 것은
**비율**뿐이고 그 비율은 문턱·HV 이력을 그대로 따른다 (trend 의 계단 = THR 4340/4341/4344/4347). ② 패널 AND 는 봉우리에서도 80 % 를 못 넘고
포화 영역에서 더 떨어진다 — 큰 샤워는 옆면(살아 있는 패널이 적은 쪽)으로 새거나 SADC 가 함께 포화·오버플로한다. 강한 veto 는 96 % 까지 올린다.
③ dst/ 의 4237 은 **스키마 1(T_Sat 없음)** 이라 6 월의 '미태그 포화 뮤온' 이 빠져 >20k 100 % 로 나왔다 → `/Data_ssd/LOG/dst4237-rebuild.sh` 로 파형부터
다시 만들었다(8.2 h, sat 832,337, 옛 캐시는 `cache.schema1-4237/`). ✅ 재생성 뒤 6 월 : >3k 태그 51.7 % · >20k 59.7 % (8·9 월보다 낮다 = 옛 '50 %' 문턱 시절),
전체 55.4 / 65.1 %. 6 월의 봉우리가 8·9 월보다 35 % 높다(하루 8.8 만 대 8.3~8.9 만은 같으니 모양 차이 — 그때 PMT 이득/HV 가 달랐다).
...
`BuildDaily.C` 의 `fn_norm_mode = 2` : on − 우발 prompt 를 두 대조 구간(`fn_fit_ranges_mev = 4.2,7.0,9.6,12.0` — IBD 봉우리 3~4 와 n-Gd 8 MeV
언덕 7.6~9.4 를 피한다)에서 A·exp(−E/λ) 로 적합(빈 오차 √(on + s²·off))해 신호창 1.2~12 전체에 편다. 모양 히스토그램 `hFnShape` 하나를 표(창 몫)·
그림 37~48·60/61 이 같이 쓴다. 대조 구간의 기대 IBD(~4 개)는 fast-n(~250)의 2 % 라 무시. 시험 monitor-daily 28 → 32. 운영 monitorcuts 를 2 로 (백업 `.bak-20260916-fn2`).
```
n-Gd, 43 일 (34.2 라이브 일)     평평 (모드 1, 8.5~12 꼬리)      지수 (모드 2)
적합                              —                              λ = 7.08 ± 1.04 MeV, A 36.0, χ²/ndf 26.5/24 (26 점)
fast-n 총량 (신호창)              846                            842        ← 총량은 같다. 모양만 낮은 에너지로 기운다
창 3~7 MeV fast-n 몫              313 (0.370)                    360 (0.428)
창 후보                           115 ± 24 = 3.36 ± 0.69 /day    68 ± 24 = 1.99 ± 0.69 /day
창 / 기대(0.20 /day, 6.8 개)      17                             10
잔여 모양 (그림 60)               3~4 MeV +25/빈, 5~6 MeV −5/빈   3~4 MeV +10/빈, 4~7 MeV ≈ 0
```
**읽는 법.** 지수 모양이 4~7 MeV 의 잔여를 0 으로 맞추고 창 후보를 절반으로 내렸다(2.9σ). 남은 것은 3.0~3.8 MeV 의 ~60 개 — 원자로 템플릿이 최대인 자리이지만
2.2 MeV(n-H γ) 언덕의 꼬리가 3 MeV 위로 새는 것(예 : 포획 γ 두 개가 겹친 사건)과 통계로는 못 가른다. 기대(6.8)의 10 배이므로 여전히 배경이 주도한다.
남은 손잡이 : 원자로 on/off 대조(§11.201)와 통계. 그림 41 이 지수 fast-n 성분을, 60/62 가 창 검수·창 rate 를 보인다 (웹 폴더 동기화 03:4x).
...
계산하라. 검출기는 RENE PTEP 논문(doi 10.1093/ptep/ptaf101 = arXiv:2507.22376) 참고, NEOS 타겟의 1/4 쯤." → `tools/reactor/expected_ibd.py`.

```
거리(m)  1 호기 1563.8 · 2 호기 1460.8 · 3 호기 1397.8 · 4 호기 1380.1 · 5 호기 1409.4 · 6 호기 1483.0      (좌표는 m 단위, 호기 간격 256 m)
열출력   1·2 호기 2,775 MWth (WH 3-loop) · 3~6 호기 2,815 MWth (OPR-1000) = 16.8 GWth (논문 평균 16.4)
타겟     Gd-LAB 270 L (논문 Table, 1200 mm × ⌀534 mm) ≈ 232 kg → 양성자 1.71×10²⁸ (NEOS 1,008 L 의 27 % — 사용자 짐작 1/4 과 같다)
ν̄ 방출   3.05×10¹⁹ fission/s/GWth × 6 ν̄ = 1.8×10²⁰ ν̄/s/GWth.  σ_f = 5.9×10⁻⁴³ cm²/fission (Daya Bay)
효율     논문 "24 m 에서 하루 ~300 IBD" 를 같은 식(효율 없이 1,030/day)으로 역산 → 0.29 (NEOS 관측률로 맞춘 선택 효율)
KHNP 실시간 (18:48, POST /branch-operation-info-by-plant, branchCd BR0303 · 2311~2336)
        1 호기 정지 (28 차 계획예방정비 2025-12-09 ~ 2027-03-31, 계속운전 설비개선) · 2 호기 정지 (28 차, 2026-09-11 ~ 2027-03-25)
        3·4·5·6 호기 원자로 출력 100 %, 발전기 1,034~1,041 MW
결과     flux 8.2×10⁹ ν̄/cm²/s (6 호기 전부면 1.17×10¹⁰) · IBD 효율 전 1.18 /day (1.70) · ★ 효율 후 0.35 /day (0.49)
        호기별(효율 후) : 3 호기 0.088 · 4 호기 0.091 · 5 호기 0.087 · 6 호기 0.079 /day
```
즉 사용자의 "하루 1 개 있을까 말까" 는 지금 원자로 상태에서는 **하루 0.35** 이고, 3~7 MeV 창(효율 0.58)에서는 **0.20 /day = 33 일에 6.6 개** 다.
§11.200 ③ 의 1.8 ± 0.6 /day 는 그 9 배 — 남는 것이 배경이라는 판정이 더 강해진다 (재생성 뒤 값으로 다시 볼 것).
4d 의 기대치를 상수 1.0 에서 monitorcuts `daily_expected_ibd_per_day`(운영 0.345, 백업 `.bak-20260915-exp`) 로 바꿨다 — 호기 상태가 바뀌면 이 도구를 다시 돌려 갱신.
계통 : σ_f ±3 % · 타겟 밀도/H 분율 ±5 % · 효율은 NEOS 선택 기준을 그대로 옮긴 것이라 우리 컷(강한 veto −12 %, multiplicity, 3~7 MeV 창)과는 별도로 곱해야 한다.
...
**✅ 재생성 결과 (09-16 03:00, `m2-rebuild.log`)** — 24 런의 서브런 28,117 개를 파형부터 다시 만들었다 (16:00 → 02:37, 3 병렬). 가드가 뺀 채널 :
4237 은 서브런 7,382 개에서 ch 2·3·7·13·16·20, 4280~4307 은 전 서브런에서 **ch29** (09-01 HV 사고 뒤 08~09 월 초 내내 pedestal 이 컷 위였다), 4345 는 0.
```
                       재생성 전 (가드 없음)          재생성 뒤 (pedestal 가드)
4237 single (mode 0 69.4 M)   29.1 M (42 %)              61.2 M (88 %)
4288 nGd  ibd / acci / single  44 / 9 / 4.10 M            67 / 12 / 6.68 M     (mode 0 single 7.33 M)
4305                           46 / 6 / 5.31 M            70 / 11 / 6.89 M
4291                           18 / 2 / 2.00 M            44 / 9 / 3.96 M
4345 (가드 안 걸림)            77 / 19 / 6.94 M           77 / 19 / 6.94 M     ← 그대로
날짜별 n-Gd (43 일, 34.2 라이브 일)  on 1,994 · 우발 452 · fast-n 660 · Li/He 23 → 858     on 2,656 · 우발 605 · fast-n 846 · Li/He 39 → 1,166
창 3~7 MeV                     on 361 · 우발 49.5 · fast-n 244 · Li/He 7.9 → 59 ± 20 = 1.8 ± 0.6 /day (33.0 d)
                                                          on 503 · 우발 61.4 · fast-n 313 · Li/He 13.4 → 115 ± 24 = 3.36 ± 0.69 /day (34.2 d)
기대 (§11.201, 0.345 × 창 효율 0.576)                    0.20 /day = 34.2 일에 6.8  →  창/기대 = 17
창 안 쌍 Δt (우발 고정 + τ=34 µs 고정) 상관 293           상관 416 (= 창 on 의 83 %, IBD 기대 6.8 은 그 2 %)
6 월(4237) 날짜별 창 rate      −7 /day 로 4 일 (single 0)  2~14 /day, 음수 없음 (그림 62)
```
**판정 (재생성 뒤).** 창 안 쌍의 83 % 가 τ = 34 µs 의 상관 쌍이고 기대 IBD 는 그 2 % 다 — 남는 3.4 /day 는 fast-n 뺄셈의 모양 오차다 : 그림 60 에서 잔여가
3~4 MeV 에 +25/빈, 5~6 MeV 에 −5/빈 이라 fast-n prompt 는 평평하지 않고 에너지가 낮을수록 많다(양성자 되튐 스펙트럼). 8.5~12 MeV 꼬리로 규격화한
평평 모양은 3~4 MeV 를 덜 빼고 5~6 MeV 를 더 뺀다. **다음 손잡이** : ① fast-n 모양을 평평 대신 지수(4.2~7 + 9.6~12 MeV 에서 기울기 적합)로,
② 원자로 on/off 대조 — 2 호기가 09-11 에 섰고(0.08 /day 몫) 1 호기는 2027-03 까지 서 있어 6 호기 전부일 때(0.49)와 비교할 자료가 쌓인다,
③ 통계 — 이 S/B(2 %) 로는 창 후보의 통계 오차(±0.7 /day) 가 기대(0.2 /day) 보다 커서 하루 단위 검증은 불가능하고 달 단위 합만 뜻이 있다.
...
조금 높였을 때 이벤트가 너무 적어 DAQ 장치들이 오류를 냈다 — 이번에도 나는지 염두에 두고 나면 정밀 조사할 수 있게 하라."
**§11.196 의 10:3x 줄(550 500 …)은 적용되기 전에 대체됐다** — 어느 런에도 기록되지 않는다 (설정 파일에 '적용 안 됨' 으로 남김).

**확인 (PRD 의 S_THR · S_PmtID, `docs/THRESHOLD-HISTORY.md`)** — run 4222 직전 구간 **4183~4221 (2026-05-28 ~ 06-10)** 의 값이
`356 164 182 293 245 335 257 443 200 1992 1032 877 125 1762 1963 251 384 187 454 200 505 205 224 215 1192 1237 857 295 597 1228` 이고, 이것을 **기준값(REF)** 으로
잡았다. 채널→PMT ID 는 2025-10 (run 2461) 부터 지금까지 같다 (`38 27 9 24 15 30 4 5 2 18 1 3 12 33 7 10 19 8 29 28 11 26 23 25 13 21 17 16 6 20`). 2025-10 ~ 2026-05 에는
같은 값들이 다른 채널 순서로 있었다 (2026-04-14 · 05-28 에 재배치). 표는 **`docs/VETO-THRESHOLD-REFERENCE.md`** (ch · PMT ID · REF · 옛 순서 · 50 % · MIP/2 · 80 %).

**설정** — `DataTaking_IBD_sykim_2026.config` SADCT THR = REF × 0.8 (반올림) :
```
285 131 146 234 196 268 206 354 160 1594 826 702 100 1410 1570 201 307 150 363 160 404 164 179 172 954 990 686 236 478 982
```
적용 run 4347 (15:56). `veto-check-4347.sh` 가 패널 반응을 남기고, 시트 Description 은 `비토 문턱값 변경(증가)` 가 붙는다 (직전 4346 대비 오른 채널 24 · 내린 6).

**★ 물리적 결과를 미리 적어 둔다 (§11.195 실측 근거).** REF 가 1000 을 넘는 채널(ch9·10·13·14·24·25·29)은 80 % 에서도 MIP(S_ADC 300~800)보다 높아
그 패널(4·5·6·7·12·14)은 뮤온도 거의 못 세고, REF 가 200 안팎인 채널은 MIP/2 아래라 γ·잡음을 채널당 100 Hz 넘게 센다 — 즉 4183~4221 시절의 모습으로
돌아간다. 사용자가 DAQ 안정성(이벤트가 적으면 오류)을 우선해 고른 값이므로 그대로 적용한다.

**DAQ 오류 감시** — `/Data_ssd/LOG/daqerr-watch.sh` (setsid, 48 h). 60 s 마다 heartbeat 와 그 런의 TCB/FADCDAQ/SADCDAQ 로그에서 `LIBUSB_ERROR ·
TIMEOUT · OVERFLOW · open falied · Fail to align · read/write error` 를 세고, 늘면 그 순간의 증거(세 로그 · heartbeat · rcsupervisor 끝 · dmesg · lsusb · 포트 ·
설정 파일)를 `/Data_ssd/LOG/daqerr-<run>-<시각>/` 에 복사하고 책임자에게 런당 한 번 메일. 로그 `daqerr-watch.log`. 감시자·chainwatch 는 그대로 돈다.
`chain_min_rate`·`recover_min_rate` 100 Hz 는 그대로 둔다 (계수율이 다시 1 kHz 대가 되면 더 안전한 쪽).

#### 11.196 ★★ '산' 패널 16 채널의 SADC 문턱을 뮤온율로 올렸다 (사용자 지시, 09-14 10:3x) — run 4347 부터

`/home/frontend/ConfigFiles/DataTaking_IBD_sykim_2026.config` SADCT THR (백업 `.bak-20260914`, 옛 줄은 주석으로). 적용은 다음 로테이션 **run 4347 (09-14 15:56)**.
```
새  550 500 550 550 500 550 500 500 230 300 500 400 250 280 280 300 600 450 200 200 300 300 500 450 700 900 280 280 200 180
옛  180 100 100 150 150 150 200 200 230 300 220 180 250 280 280 300 220 100 200 200 300 300 100 100 150 150 280 280 200 180
```
**값을 정한 법 (run 4345 sub 100~104, 300 s, `scratchpad/VetoThr.C`)** — THR 은 S_ADC 단위다(트리거된 사건의 S_ADC 1 % 분위수 = THR, `VetoThrUnit.C`).
채널마다 '트리거된 사건의 S_ADC 가 T 를 넘는 수' 를 300 s 로 나눠 **1~2 Hz 가 되는 T** 를 고르되, 타겟 관통 뮤온(>3000 NPE) 동시 사건의
S_ADC 25 % 분위수(MIP 분포의 아래쪽)를 넘지 않게 했다 — 뮤온의 75 % 이상은 남긴다. 예 : ch0 >500 630 (2.1 Hz) · >600 227 (0.8 Hz), MIP p25 638 → 550.
ch24·25 는 이득이 높아(MIP p50 1400 · 1156) 700 · 900. **패널 4·6·7·9·10·13·14 의 14 채널은 그대로** — 이미 뮤온만 센다 (§11.195).
기대 : veto 패널 AND 율 채널당 1~2 Hz, veto 전체 ~10 Hz 안팎, **DAQ 총 계수율 1055 → ~200 Hz** (FADC 185 + 뮤온). 확인은 `/Data_ssd/LOG/veto-check-4347.sh`
가 PRD 12 개 뒤 `veto-check-4347.log` 에 남긴다. 판정 기준은 이제 '패널 % 가 얼마나 높은가' 가 아니라 **'Hz 가 뮤온율에 가까운가'** 다.
...
날짜·누가·적용 런·왜·근거·백업 — append-only, 2026-04-17(run 3950) 부터 오늘까지 8 건. ③ 설정 파일의 SADCT THR 주석 줄(옛 줄을 지우지 않고 주석으로 남기며
적용 런 구간을 적는다). **문턱을 바꾸면 셋 다.** (§11.167 의 4,213 개 설정 파일 대조 표 `thr_history.tsv` 는 3950~3980 · 4200~4334 만 덮는 부분 표였다.)

#### 11.195 ★★★ "죽은 veto 패널 7 개" 는 틀렸다 — 그 일곱이 유일하게 뮤온을 세고 있다 (사용자 질문, 09-14 05:0x)

사용자 : "검출기가 동굴(RENO 원거리 터널, 450 m.w.e.) 안이라 우주선이 적을 수밖에 없다. RENO 논문의 뮤온율을 찾아 적용하거나 450 m 물 아래로
환산해 지금 트리거보다 높은지 계산하고, veto 가 죽은 게 맞는지 확신할 증거를 가져와라."

**문헌** — RENO ⁹Li/⁸He 논문 (PRD 106, 012005 (2022) · arXiv:2204.09215) : 뮤온율 **near 117.5 Hz (120 m.w.e.) · far 13.1 Hz (450 m.w.e.)**, 평균 에너지 33.1 / 73.6 GeV.
RENO 검출기 내부(buffer 5.4 m 지름 × 5.8 m) 단면 ≈ 25~30 m² 이므로 far 의 뮤온 플럭스 ≈ **0.4~0.5 /m²/s**. RENE veto 패널은 Type A 2.5 × 0.6 m (1.5 m²,
위·긴 옆면) · Type B 1.66 × 0.6 m (1.0 m², 앞·뒤) (RENE PTEP 2025, arXiv:2507.22376). **→ 패널 하나에 기대되는 뮤온율은 0.3~0.7 Hz, veto 전체 5~8 Hz, 타겟 몇 Hz.**

**실측 (run 4345 서브런 100~104, 300 s, 322,409 사건, 읽기 전용 `scratchpad/VetoAlive.C` · `VetoPanel.C`)**
```
타겟 > 3000 NPE (타겟 관통 뮤온) 951 = 3.2 Hz · > 20000 NPE (샤워) 331 = 1.1 Hz          ← 450 m.w.e. 기대와 같은 크기

패널(채널 짝 AND)      AND 수 / 300 s    Hz     그중 타겟 > 3000 NPE 동시   트리거 때 S_ADC 중앙값
 '살아 있다' 던 8 개    5,371 ~ 81,970   18~273     0.1 ~ 0.8 %              130 ~ 280  (문턱 바로 위)
 '죽었다' 던 7 개
   4 (ch8,9 · PMT 2,18)       294        0.98      30 %                      389 / 446
   6 (ch12,13 · 12,33)        444        1.48      21 %                      339 / 372
   7 (ch14,15 · 7,10)         280        0.93      31 %                      526 / 593
   9 (ch18,19 · 29,28)        127        0.42       7 %                      482 / 373
  10 (ch20,21 · 11,26)         82        0.27       7 %                      674 / 814
  13 (ch26,27 · 17,16)        132        0.44      13 %                      636 / 518
  14 (ch28,29 · 6,20)          88        0.29      16 %                      474 / 321
타겟 샤워(>20000 NPE) 331 건에서 켜진 비율 : 패널 4 · 6 · 7 의 채널 11~18 %  =  '살아 있다' 던 패널 0 · 1 · 5 (10~18 %) 와 같다.  9·10·13·14 는 0.3~4 %
'죽은' 패널만 켜진 사건 977 (3.3 Hz) 중 타겟 뮤온 동시 164 (17 %)  ↔  '산' 패널만 켜진 사건 260,573 (869 Hz) 중 269 (0.1 %)
```
**결론.** ① 일곱 패널은 죽지 않았다. 450 m.w.e. 에서 기대되는 그 값(0.3~1.5 Hz)으로 트리거되고, 신호가 MIP 크기(ADC 300~800)이며,
트리거의 7~31 % 가 타겟 관통 뮤온과 동시다. 타겟 샤워에 대한 반응도 다른 패널과 같다. **이것이 제대로 된 뮤온 카운터의 모습이다.**
② 반대로 '살아 있다' 던 여덟 패널은 뮤온율의 **100 배**(18~273 Hz)로 트리거되고 그 99.9 % 가 타겟 뮤온과 무관하며 ADC 가 문턱 바로 위다 —
γ 배경·잡음·빛샘 같은 **뮤온이 아닌 것**이다. 864 Hz 의 'veto 뮤온율' 은 99 % 가 이것이고, 150 µs after-muon 컷은 그 위에 12 % 라이브타임을
버리고 있었다. ③ §11.166~11.178 의 "패널 7 개가 죽었다 · MIP/2 문턱이 패널을 죽였다 · 문턱을 100 으로 되돌려라" 는 전부 이 오독에서 나왔다.
MIP/2 로 올렸을 때 패널 11 이 0.7 Hz 가 된 것은 죽은 것이 아니라 **비로소 뮤온만 세기 시작한 것**이었다. **되돌린 다섯 채널(1·2·17·22·23)의 문턱은
다시 올려야 한다** — 기준은 '패널 AND 율이 0.3~1.5 Hz 로 내려오는 값'. ④ 강한 veto 배치(mode 2, §11.194)는 그 잡음까지 더 잡는 쪽이라 라이브타임만
더 잃는다(사용자가 감수한다고 했으므로 그대로 두되, 결과 해석에 이 사실을 넣을 것). 물리적으로 맞는 오프라인 veto 는 **채널마다 MIP/2 이상의 S_ADC**
(mode 3 후보)다. ⑤ 사용자 질문 그대로 답하면 : RENO far 13.1 Hz 를 RENE 로 환산한 기대 트리거는 패널당 1 Hz 안팎이고, 지금 '살아 있다' 는
패널의 트리거는 그보다 100 배 높다 — 즉 우주선이 아니다.
...
**배치를 기다리는 동안 현재(패널 AND) DST 로 잰 진단 셋 (04:0x ~ 04:3x, 그림 53~58)**
```
다중도  창 안 다른 single 수 n 의 on−acc 초과분 (n-Gd) : N(0)=2,250 · N(1)=1,769 · N(2)=3,543 · N(3~4)≈3,300~3,700 → 뮤온 파쇄 다발이 평균 4 개쯤(포아송 μ≈4).
        외삽 N(0) 의 다중중성자 몫 = N(1)²/(2N(2)) = 441 ± 27  ↔ 남는 '신호' 1,572.  즉 다중중성자 가족은 남는 것의 1/4~1/3 이다
모양    n_extra=1 · ≥2 가족의 prompt 는 2.2 MeV(n-H 포획) + 8.5 MeV(n-Gd 포획) 봉우리.  남는 '신호' 는 2.2 봉우리를 갖되 1.2~2 MeV 가 더 크다(문턱 쪽으로 오른다)
창 확대 multiplicity 창 200/400 → 1000/1000 µs : on 3,182 → 2,708 (−15 %), 남는 것 1,572 → 1,350 (−14 %), 외삽 441 → 460.  = 무작위 손실만큼만 줄고 배경은 그대로 → 효과 없음
Δt      ★ 결정적.  AmBe run 4221 (41 서브런) 의 n-Gd 창 쌍 Δt 는 τ = 34 µs.  물리 런의 on-window 쌍은 우발 바닥(off-window)을 고정하고 적합하면
        τ = 56 µs (전체) · 70 µs (prompt < 3 MeV) 인 단일 성분이고, 34 µs 성분은 0 ± 1,575 (축퇴). 즉 후보의 Δt 모양이 Gd 포획이 아니다 —
        IBD 라면 34 µs 여야 한다. 6~10 MeV 'delayed' 가 100 µs 넘게 이어지는 것은 검출기 밖(구조물·완충재)에서 열화돼 되돌아오는 중성자나
        철 포획(7.6 MeV γ) 같은 것을 의심하게 한다. 가설이다 — 위치·PSD 정보가 없어 여기서는 못 가른다
```
**컷에 대한 의견 (사용자 질문)** — ① 강한 veto 는 태그된 뮤온에만 듣는다. 실측으로 +2~6 % 뿐이라 큰 개선을 기대하기 어렵다. 배경의 뿌리는
**veto 패널 15 개 중 7 개(4·6·7·9·10·13·14)가 죽어 있는 것**(§11.178)이므로 가장 효과적인 '컷' 은 패널 수리다. ② multiplicity 창 확대는 무효(위).
③ PSD 무효(§11.193). ④ **Δt 가 유일하게 IBD 를 가르는 축** — 34 µs 성분과 ~60 µs 성분의 2-성분 적합으로 IBD 몫을 뽑거나, Δt < 40 µs 로
자르면 IBD 69 % 를 남기고 긴 성분을 절반 넘게 뺀다. 이것을 4d 에 '2-성분 Δt 적합' 지표로 넣는 것을 제안한다 (분석팀 판단).
⑤ 다중중성자 가족의 n_extra=0 몫(441)은 표에 실어 빼는 항목으로 둘 수 있다 [prelim].

**✅ 배치 결과 (15:20 끝, 43 런 · 11 h 26 m. 15:34 daily 가 dst_m2 로 돌아 그림 32~58 을 웹에 넣었다, `daily-m2-first.log`)** — 지금부터 4d 는 dst_m2 를 읽는다.
```
                  패널 AND (dst)      강한 veto mode 2 (dst_m2)
뮤온 (run 4345)   853 Hz              884 Hz (+3.6 %)          live 계수 ×0.880 → ×0.876
on-window 쌍      3,182               1,929  (−39 %)
우발 / fast-n / Li/He   932 / 618 / 119     442 / 642 / 26
남는 것           1,572               819
우발 뺀 후보 rate  61 /day             46 /day   (live 36.6 d → 32.1 d)
Δt 적합 τ (우발 바닥 고정)  56 µs (E<3 : 70)   47 µs (E<3 : 49)   — AmBe 의 34 µs 쪽으로 다가갔으나 아직 위
다중도 외삽 N0    441                 350
```
남는 819 의 prompt 는 여전히 1.2~3 MeV 에 몰리고 8 MeV 봉우리도 작게 남는다. n-H 의 다중도 표는 off-window 규격화가 커서 N(1) 이 음수로 나온다(진단용, 무시).
...
중성미자 신호 스펙트럼이 너무 이상하다." **런별 파이프라인은 그대로 두고 4d(BuildDaily) 에만 더했다.**

**그림 12 장 추가 (41~52, `tools/monitor/README.md` 표)** — 41/43 prompt · 42/44 delayed 의 **배경 성분별 스펙트럼** (우발 · fast-n · Li/He 템플릿 ·
multiplicity 기각 초과분 · (켜면) PSD/뮤온 veto 로 버린 것 · 사이드밴드 원 prompt 0~50 MeV), 45~48 **신호창 분해** (전체 = 신호 + 배경 쌓기),
49/50 prompt **p_psd** on 대 off, 51/52 **샤워링 뮤온 뒤 Δt** (직전 · 직후 대조 · 합의 Eq.2 적합). 히스토그램은 전부 `daily_spectra.root` 에.

**배경 조건 (정본 `/home/ojk/analysis3/essential/AnalysisCondition.h`, DST 단계는 `dst-build.sh`)** — README 4d 절에 표로 적었다.
```
DST 단계   muon veto = SADC 패널 위/아래 동시 · after-muon 150 µs · 포화 제거(T_Sat 에 따로 보관) · single = q0+q1 > 1.2 MeV
쌍         prompt 1.2–12 MeV · delayed n-Gd 6–10 / n-H 1.87–2.59 MeV · Δt n-Gd [1,100] / n-H [2,400] µs
multiplicity  prompt 앞 200(600) µs · delayed 뒤 400(1000) µs 에 1.2 MeV 넘는 single 없음 + 창 안에 다른 single 없음   (괄호 = n-H)
우발       off-window [1000, 1000+Δt_max] µs 쌍 × (Δt_max−Δt_min)/Δt_max
fast-n     사이드밴드 prompt 12–50 MeV (single ∪ 포화) 쌍 × 신호창 폭/사이드밴드 폭, 평평
Li/He      직전 샤워링 뮤온(> 20000 NPE)까지 Δt 를 Daya Bay Eq.2 로 날짜별 적합 (R_μ 고정, Li 분율 1)
```

**★ 실측이 가른 것 (43 런 · 41 일, n-Gd)**

| | 사이드밴드 평평(기존) | + 꼬리 규격화 + PSD 3σ | + 300 µs 뮤온 veto |
|---|---|---|---|
| on-window 쌍 | 3,182 | 3,175 | 2,744 (live −11 %) |
| 우발 | 931.6 | 931.6 | 819.7 |
| fast-n | **921.4** | **618.0** | 519.0 |
| Li/He (날짜별 합) | 119.0 | 118.0 | 43.4 |
| 남는 것 | 1,210 | 1,507 | 1,362 |
| 우발 뺀 후보 rate | 61.4 /day | 61.2 /day | 59.0 /day |

```
① fast-n 사이드밴드는 못 쓴다.  사이드밴드 쌍 3,242 개의 prompt 가 전부 30 MeV 한 빈 = 포화 사건(NpeToMeV 상한) 이다. 새는 뮤온·샤워라
   신호창으로 평평하게 외삽하면 4 MeV 위 꼬리(빈당 ~8) 보다 큰 값(빈당 ~17)을 빼 스펙트럼이 4 MeV 위에서 사라졌다 — 그것이 '이상한' 모양의 첫째 원인.
   → fn_norm_mode 1 : 신호창 안 8.5–12 MeV 꼬리(우발 뺀 on 쌍)로 평평한 높이를 정한다. 921 → 618. 사이드밴드는 그림 41/43 에 대조용으로만
② PSD 컷은 이 분리력(FoM 0.5)으로는 효과가 없다. 3σ 에서 3,175 중 7 쌍 (n-H 110/2.4 M). 그림 49/50 의 on/off 분포가 겹친다
③ 뮤온 veto 를 150 → 300 µs 로 늘려도 후보 rate 는 61.2 → 59.0 /day 로 거의 그대로(줄어든 것은 Li/He 몫)이고 라이브타임 11 % 를 잃는다. 안 켠다
④ ★ 남는 스펙트럼의 모양 자체가 문제다.  우발·fast-n·Li/He 를 빼고 남는 1,507 쌍의 prompt 는 1.2–3 MeV 에 몰려 있고(1,100), 7.5–9.5 MeV 에
   **n-Gd 포획 γ 봉우리(~100)** 가 있다. IBD prompt 는 3–4 MeV 에서 봉우리를 이루고 8 MeV 위는 거의 없어야 한다. 그리고 multiplicity 가 걸러낸
   초과분 19,708 쌍(그림 41 주황)이 **정확히 같은 두 봉우리(2.2 MeV n-H 포획 · 8.5 MeV n-Gd 포획)** 를 보인다 — 뮤온 파쇄 중성자 여럿이 잇달아
   포획되는 사건(첫 포획이 prompt, 다음 포획이 delayed). 중성자 셋 이상이면 multiplicity 가 잡지만 **둘이면 그대로 통과**해 남는 스펙트럼을 채운다.
   즉 지금 '신호' 의 대부분은 2-중성자 상관 배경이다. 이것은 컷으로 못 빼고(IBD 와 구별 불가) **multiplicity 기각 표본(3-중성자 이상)에서
   포아송 다중도로 2-중성자 몫을 외삽해 빼야 한다** — 분석팀 몫으로 넘긴다. 반대로 n-H 는 우발이 92 % 라 남는 195 k 도 우발 통계 오차 안이다
```

**손잡이 (monitorcuts.params, 4d 만 읽는다)** — `daily_psd_nsig`(−1 끔) · `fn_norm_mode`(0/1) · `fn_norm_lo_mev`(8.5) · `mu_veto_us`(0) · `shower_veto_ms`(0).
운영 디렉터리에 `config/monitorcuts.params` 를 처음 만들어 **`fn_norm_mode = 1` 만 켰다** (나머지는 기본값 = metrics 와 같다). 표에 `n_psd_rej` · `fn_mode` · `n_mu_rej` 열.
시험 `tests/monitor-daily.test.sh` 10 → **20** (그림 21 장 · 열 · 컷 회계 on = 남은 것 + 버린 것 · 라이브타임 보정). 실자료 세 벌은 scratchpad 의 plot-try{,2,3}.
...
★ 001742 의 PRD 는 두 벌이다 — ZK206KFR 의 001742_2 (1,615 개) 와 ZK206K89 의 001742 (1,613 개). 사용자 확인 (00:2x, 서브런 수로 안다).
  docs/backup-disks/run-notes.tsv 에 (런, uuid8) 손 메모로 적어 두면 재작성 때 Notes 열에 ★ 로 붙는다
RENE-ALL-019 ZFL3GKNN (06-06)      001931~002406 의 런 422 개 · 137,929 파일 · 1.78 TB (2026-06 한 달치. 서브런 0 이 아닌 곳에서 시작하는 런 다수)
RENE-PRD-001 ZK206L7N (08-26)      ★ 런 폴더 없이 RENE_data_backup 바로 밑에 PRD/ (002442 의 PRD 8,285 개 · 1.79 TB) 와 Merged/ (338 개, 0 바이트 25
                                   = §11.123 의 08-27 이탈 잔재). 스캐너가 이 '느슨한 배치' 를 파일 이름에서 런을 읽어 한 줄(layout=loose)로 낸다.
                                   PRD 만 담았으므로 첫 PRD 하드 라벨. 시리얼이 §11.124 의 ZK2060HX 와 다르다 — 그때 값은 lsblk 였을 수 있다
★ 밟은 것 : 첫 스캔이 Merged/PRD 를 런 이름으로 냈고, 그 잘못된 행이 다음 재작성에서 '옮긴 옛 행' 으로 살아남아 라벨을 ALL 로 만들었다.
  → 옛 스캔이 만든 행(Notes 'from disk scan')은 다시 못 만들면 옮기지 않는다 (시험 54). disks.tsv 의 그 줄을 지우고 다시 돌려 PRD-001 로
```
**시트 1,386 행 · 하드 21 장 (00:27).**

**00:31 두 장 더 — 둘 다 002442 (§11.123 의 그 하드들). 종류 MERGED 를 추가했다.**
```
RENE-MERGED-001 ZK205ZZF (UUID ad07fc77 = §11.123, 포맷 08-17)  002442 의 Merged 8,183 개 · 1.84 TB (08-17~26, --remove-source-files 로 옮긴 것). 캐시 (§11.149)
RENE-RAW-002    ZK2060HX (UUID 26f4448e = §11.124, 포맷 08-27)  002442 의 FADC 8,546 (서브런 3~8548) + SADC 537 개 = 1.87 TB.  ★ SADC 가 일부뿐 — RAW 전체(17,098)는 KHU 에
→ 002442 는 하드 세 장에 걸쳐 Merged(ZK205ZZF) · RAW(ZK2060HX, 불완전) · PRD(ZK206L7N 8,285 / KHU 8,548) 로 나뉘어 있다. run-notes.tsv 에 적었다.
   09-01 의 '002442 완료' 기록(UUID 834a597b) 하드는 아직 안 꽂혔다
```
**★★ 사고 — 시트가 통째로 비었다 (00:32).** 재작성이 `ws.clear()` 뒤 `ws.update()` 를 하는데, 1,388 행 쓰기가 API 오류로 실패해 빈 시트만 남았다
(다음 실행이 `[FATAL] 헤더가 기대와 다르다 : []`). 손 열(KHU · 메모)은 시트에만 있었으므로 **쓰기 직전에 남긴 백업 TSV** 가 유일한 원본이었다.
→ 도구를 **먼저 쓰고 남는 아랫줄만 지우는** 순서로 바꿨고(`batch_clear`), 시트가 비어 있으면 `--old-sheet-tsv <백업>` 에서 옛 행을 살린다.
`sheet-before-rebuild-20260913003108.tsv` 로 복구 → 1,388 행 · 하드 23 · 되읽어 대조 통과 · 손 열 확인. 시험 60. **백업 TSV 를 지우지 말 것.**
시리얼 `?` 는 여섯 장 그대로 (834a597b · E · F · d215edd1 · 73329c97 · 0b455e98…).

**00:41 두 장 더 — ★★ §11.170 의 A·B 가 실물로 있었다. '재포맷됐다' 는 시트 메모(09-05)가 틀렸다.**
```
RENE-ALL-020 ZK20691W  UUID e39bd49a (A)  포맷 09-03 00:18  002443 조각 1 : 8,696 개 (FADC 8,537 · PNG 156 · Merged 2 · 서브런 0~10536) · 1.87 TB  = parts_index 그대로
RENE-ALL-021 ZK206KGB  UUID 09387574 (B)  포맷 09-03 00:19  002443 조각 2 : 8,061 개 (FADC 2,106 · Merged 5,952 (§11.152 전이라 Merged 포함) · 서브런 8534~10642) · 1.87 TB = parts_index 그대로
→ C(ZK206K95) · D(ZFL3GHHS) 는 별개 하드다. 002443 은 A·B·C·D 네 장에 온전히 있고 KHU 로 되살릴 필요가 없다.
   aliases.tsv 의 두 줄을 지웠고(파일은 형식만 남김), run-notes.tsv 에 '메모가 틀렸다' 를 적어 시트 Notes 에 붙였다. §11.187 의 'A·B 는 재포맷' 문단은 이것으로 정정
```
**★★ 사고 둘째 — Notes 가 재작성마다 두 배로 자랐다.** 손 열 보존이 옛 Notes 전체를 `hand:` 로 다시 붙여, 여덟 번 재작성 뒤 한 칸이 50,000 자를 넘겨
`APIError 400` (쓰기 전에 거부돼 시트는 무사). → 옛 Notes 에서 **사람이 적은 조각만**(자동 문구 접두어 제외, `hand:` 겹침 벗김) 한 번 붙이고,
'옮긴 행' 표시도 한 번만. 시험 62 (세 번 재작성해도 바이트 동일). 시트 1,388 행 · 하드 25.

**00:43 두 장 더 (시트 1,425 행 · 하드 26)**
```
RENE-ALL-022 ZFL3GH36  UUID 71e20b6e  포맷 08-06  002407~002440 의 런 34 개 (RAW+PRD+Merged+PNG, 08-06~17 복사) + 느슨한 Merged/ 에 002444 의 Merged 1,850 개
RENE-ALL-004 ZK206L1T  UUID 834a597b  포맷 09-01  = 시트 1~29 행의 그 하드 (code7/8 시절). 002442 의 SADC 8,019 + PNG 8,547 + PRD 263 (09-01 'full')
                                                   + 09-02 의 작은 런 28 개 + ★ 002456 통째 7,646 개 (사용자의 09-02 code8 세션) + ★ 002443 FADC 126 개 (내 09-02 시험 세션, §11.141)
★ 002456 은 09-12 에 서버에서 다시 옮겨졌으므로(ZK206014 조각 + Z4ZBXGAA full) 두 벌이고, 002443 서브런 0~125 도 A 와 겹친다. run-notes.tsv 에 적었다
★ 밟은 것 : UUID 로만 알던 하드를 스캔해 시리얼을 알게 되면 물리키가 바뀌어 새 라벨(ALL-023)을 받았고 옛 ALL-004 는 정본에서 사라졌다.
  → 라벨 물려받기(UUID 키 → 시리얼 키) + 표에 없는 키의 라벨도 정본에 남긴다. ZK206L1T 는 ALL-004 로 되돌렸다. 시험 66
```
**00:48 두 장 더 — C·D (시리얼은 이미 알던 값). 기록과 실물이 맞는다 (시트 1,425 행 · 하드 26 그대로)**
```
RENE-ALL-005 ZK206K95 (C, 937a91bc, 포맷 09-04 00:12)  002443 : 18,817 개 · 1.87 TB (색인 4 조각 16,066 + rc=24 세션의 Merged 2,751, 시트 33 행 메모대로)
RENE-ALL-006 ZFL3GHHS (D, 27fe8b47, 포맷 09-04 00:13)  002443 : 15,700 개 · 1.32 TB (시트 34 행) + 002444 조각 2,929 개 · 542 GB (색인 그대로)
→ 002443 은 A·B·C·D + (ZK206L1T 의 앞 126 개) 에 있다. 이제 시리얼 모르는 하드는 E · F · d215edd1 · 73329c97 네 장
```
**00:54 두 장 더 — E·F. 색인과 일치, 라벨은 UUID 때 것을 물려받았다 (시트 1,426 행 · 하드 26)**
```
RENE-ALL-007 ZK2060L8 (E, 0b455e98, 포맷 09-06 16:20)  002444 조각 14,258 개 · 서브런 2381~5303 · 1.87 TB  = 색인 그대로
RENE-ALL-008 ZK2068YX (F, a7bb5f16, 포맷 09-06 16:21)  002444 마지막 조각(full) 4,026 개 (SADC 만, 79 GB) + 002447 조각 5,535 개 · 1.79 TB = §11.170 표 그대로
→ 시리얼 모르는 하드는 d215edd1 · 73329c97 두 장 (09-09~10 의 2회차 첫 세션)
```
**00:58 두 장 더 — G·H (시리얼은 알던 값). §11.170 표 그대로 (시트 1,426 행 · 하드 26)**
```
RENE-ALL-009 ZK206APL (G, 75a4957f, 포맷 09-08 14:50)  002447 마지막 조각(full) 3,830 개 · 970 GB + 002451 조각 3,086 개 · 896 GB
RENE-ALL-010 ZK2060CP (H, 5c136765, 포맷 09-08 14:50)  002451 마지막 조각(full) 2,683 개 · 771 GB + 002452 조각 4,066 개 · 1.10 TB
```
**01:02 마지막 두 장 — 2회차 첫 세션(09-09~10)의 하드. ★ 이로써 26 장 전부 시리얼 확인. 시트 1,426 행 · 하드 26**
```
RENE-ALL-011 ZK206ATM (d215edd1, 포맷 09-09 22:36)  002452 마지막 조각(full) 1,828 개 · 212 GB + 002453 full 5,777 개 · 1.44 TB + 002454 조각 1,868 개 · 215 GB
RENE-ALL-012 ZFL3GJKH (73329c97, 포맷 09-09 22:36)  002454 마지막 조각(full) 3,977 개 · 1.43 TB + 002455 조각 3,363 개 · 436 GB
```
...
```
**01:09 마지막 쌍 (2회차의 두 장, ALL-021 · ALL-022) 도 스캔 — ★ 26 장 전부 실물 스캔 완료.** 색인과 일치 :
ZK206014 = 002455 마지막 조각(full) 5,947 + 002456 조각 3,196 · Z4ZBXGAA = 002456 마지막 조각 2,921 + 002457 full 5,825 + 002458 조각 1,171 (서브런 0~608).
시트 1,426 행. 이제 서버에 남은 백업 대상은 002458 의 나머지 4,682 개부터 런 1,681 개다 — 새 하드는 RAW/PRD 로 나눠 담는다 (§11.188 운용 순서).
...
**★ (09-13 00:41 정정 : 아래 문단은 틀렸다. A·B 는 실물로 있었다 — §11.188 끝.) §11.170 의 표를 고친다 — A·B 는 별개 하드가 아닐 가능성이 크다.** 시트 30·31 행 메모(09-05, udevadm)에 "A(e39bd49a)는 09-04 00:12 에
...
**전부 실측 (읽기 전용) 으로 확인한 뒤 코드 한 곳을 고쳤고, 책임자에게 정정 보고 메일을 보냈다.**

```
세션         09-11 23:39:45 시작 (pid 735666, 옛 판) → 09-12 17:06:58 마지막 기록.  실패 0
옮긴 것      002455 full → /backup_hdd · 002456 part 1681 + 1515 (07:04 /backup_hdd 가득) + full 나머지 → /backup_hdd_2
             002457 full → /backup_hdd_2 · 002458 part 1171 개 → /backup_hdd_2 (17:06)  ★ 마지막 조각
             하드의 002458 = 1171 개 (재귀) = 색인 1171 = 매니페스트, 원본에 그 파일 0 → 대조·삭제까지 정상
하드         /backup_hdd sdc ST2000DM008 ZK206014 (UUID 2879e725…) · /backup_hdd_2 sdd ST2000DM006 Z4ZBXGAA (UUID e84db00d…)
             둘 다 여유 2.1 G (100 %).  남은 런 1,681 개 (002458 의 나머지 4,682 개부터)
code9.log 끝  "📦 /backup_hdd_2 가 가득 찼습니다 … 1681 개 런 남았습니다"
             /home/frontend/data_backup_simple_code9.sh: line 1097: syntax error near unexpected token `)'   ← ★ 여기서 죽었다
backup_log   이 세션의 '종료 code=' 줄이 없다. 마지막 code= 줄은 09-10 18:52 (지난 세션)
```

**① 종료 메일은 오지 않았다 — 원인은 §11.185 가 짚은 제자리 덮어쓰기다.** 세션이 도는 동안 새 판을 `scp` 로 같은
파일(inode)에 네 번 덮어썼고, bash 가 루프 밖 최상위 코드(하드 소진 → `finish`)를 파일 오프셋으로 읽는 순간 어긋난
바이트를 읽어 문법 오류로 죽었다. 배포본 자체는 멀쩡하다(`bash -n` OK, md5 = 저장소 `c139ea0f`). 회차 결과 · 개수/바이트
대조 · 원본 삭제 · `parts_index` 기록(7 열, 옛 판)은 그 전에 끝났으므로 **자료 손실은 없다.** 잃은 것은 종합 메일 한 통과
`code=3` 로그 줄뿐이다. **§8 의 '`.new` + `mv`' 규칙이 바로 이 사고의 재발 방지다.**

**② cron 이 대신 보낸 '세션 끝남' 메일(17:37)은 틀린 문구를 실었다.** `backup-sheetlog.sh` 가 `backup_log.txt` 의
마지막 `code=` 줄을 무조건 인용해 `[Thu Sep 10 06:52:05 PM] 종료 code=3 : 하드를 다 썼습니다` 가 붙었다 — 지난 세션 것인데
정상 종료처럼 읽힌다. **고침** : 세션을 발견할 때 시작 시각을 state 에 `session_start=` 로 남기고, 끝날 때는 그 뒤에 찍힌
`code=` 줄만 인용한다. 없으면 **"★ 종료 기록 없음 — 스크립트가 마무리(종합 메일)를 못 남기고 죽은 것"** + 로그 마지막 줄.
시작/종료 알림 둘 다에 기록 시트 링크(`SHEET_URL`, `BACKUP_SHEET_URL` 로 갈아끼움)를 넣었다. 시험 40 → **45 통과**.

**③ 메일에 시트 링크가 없던 이유 — 이번 세션의 메일은 전부 옛 판이 보낸 것이다.** 07:04 '가득 참' 메일도 옛 판이라
`SHEET_URL` 이 없다. 새 판은 다음 세션부터 모든 본문에 넣는다(§11.183). cron 알림에도 ②로 넣었으니 이제 세션 시작 알림부터 링크가 있다.

**④ 시트 자동 등재는 이미 돌고 있다 — 설정할 것이 없다.** `crontab` `37 * * * * scripts/backup-sheetlog.sh` (§11.184).
이번 세션의 조각·완료가 전부 올라가 있다 : 시트 53 행, 마지막 `2026-09-12 17:06:13 · 002458 · 1171 개 · /backup_hdd_2`.
18:37 · 19:37 · 20:37 은 "새 행 0" — 새 기록이 없으니 정상이다. 사람이 할 것은 `Disk Label` · `Storage Location` 두 열뿐.

**책임자 메일** `[RENE DAQ] 저장소 백업 세션 종료 보고 — 하드 2장 가득, 스크립트 종료 메일 누락 원인과 정정` (routine, rc=0) —
위 ①~④ 와 이어가는 순서(umount → 하드 교체 → by-id 마운트 → chown → `--dry-run` → 같은 명령 → cron 이 `build=NEW` 알림).
...
backup-sheetlog 도 07:37 · 10:37 · 15:37 회차가 세션 없이 스스로 행을 붙였다(시트 52 행).

**★ 오보 하나와 그 뿌리 — scp 는 제자리 덮어쓰기다 (같은 inode).** 첫 실행이 현재 백업 세션(09-11 23:39 시작, 옛 판)을
`build=NEW` 로 알렸다. 경로를 읽어도, `/proc/<pid>/fd/255` 를 읽어도 새 내용이 보였다 — **scp 가 파일을 제자리에서 덮어써
inode 가 그대로**(1610622041)라서다. 즉 09-12 에 code9 를 네 번 배포하는 동안 **도는 세션이 붙든 파일의 내용이 네 번 바뀌었다.**
bash 는 시작 때 파싱한 함수와 실행 중인 루프는 메모리에서 돌지만, 루프를 빠져나온 뒤의 최상위 코드는 파일의 바이트 오프셋으로
읽으므로 **세션 끝의 `case OUTCOME`/finish 가 어긋난 자리를 읽을 수 있다** (§11.42 의 그 위험). 원본 삭제는 run_pass 안(메모리)
이라 자료는 안전하다. 세션이 끝날 때 종료 메일이 제대로 오는지 볼 것.
→ 판정을 **세션 시작 시각 ≥ 파일 mtime 이면 NEW** 로 바꿨고(40 건), 정정 메일을 책임자에게 보냈다.
→ **§8 규칙 추가 : 저장소 서버 배포는 `scp … code9.sh.new` 뒤 `mv -f`** (새 inode. git pull 이 안전한 것과 같은 이유, §11.139).
...
220 100 200 200 300 300 100 100 150 150 280 280 200 180` (ch1·2·17·22·23 = 100, 나머지 MIP/2). 백업 `.bak-20260910b`,
직전 줄은 주석으로. 적용은 run 4342 (09-11 15:47). `/Data_ssd/LOG/veto-check-4342.sh` 가 PRD 12개 뒤 패널 반응을
`veto-check-4342.log` 에 남긴다. 기대 : 패널 1 이 4333 수준(~246 Hz) 근처로, veto 총 계수 ~830 Hz.
(4342 는 부팅 실패라 확인은 run 4344 로 옮겼다, §11.179.)

**✅ 결과 (veto-check-4344, 서브런 10, 09-11 16:1x)** — 기대대로 돌아왔다. 절대 계수 Hz :

```
              4333(옛)  4340(MIP/2)  4341(4ch 되돌림)  4344(ch2 까지)
veto 전체       737        508          719             872     ← 옛 값보다 +18 % (패널 5·12 가 더해진 몫)
패널 1          246        147          135             274     ← ch2=100 으로 회복
패널 0          127         43          138             134
패널 8          108         15           77              78
패널 11          53          1           71              74
패널 12           0         99           93             101
패널 5            1         17           18              18
DAQ 총 계수     927        694          909            1057
```

되돌린 다섯 채널(1·2·17·22·23)의 패널이 모두 옛 수준 이상이고, 내린 문턱으로 살아난 5·12 는 그대로다. **문턱 조정은 여기서
마무리.** 남은 것은 문턱과 무관한 죽은 일곱 패널(4·6·7·9·10·13·14)의 현장 확인뿐이다.
...
**✅ 재검증 (09-12 15:56 로테이션 4344 → 4345)** — 24 h 완주 · exit 0 · USB 오류 0(마감·부팅 둘 다) · DRAM 8/8, 4345 ~1055 Hz.
chainwatch 가 `rotate`(간격 27초)로 판정해 **책임자에게만** 보냈다(16:00). 09-11 의 4341→4344 는 `resumed`(사이에 실패 런) 로
목록에 간 것도 맞았다. 메일 정책 §11.172 의 세 갈래가 전부 실전으로 확인됐다.

**veto-check-4341 (서브런 10 하나, 16:1x)** — 되돌린 문턱의 효과. 절대 계수 = 패널 % × 총 계수.

```
              4333(옛 문턱)   4340(MIP/2)   4341(되돌림)     비고
veto Hz         737            508            719           거의 회복. 패널 5·12 가 새로 더해진 채
패널 0  Hz      127             42            138           회복 (ch1 100)
패널 8  Hz      108             15             77           대부분 회복 (ch17 100. ch16 은 220 그대로)
패널 11 Hz       53            0.7             72           회복 (ch22·23 100)
패널 12 Hz      0.3             99             93           살아 있음 (150)
패널 5  Hz      0.5             17             18           살아 있음 (220/180)
패널 1  Hz      246            147            134           ★ 여전히 -45 %.  ch2 가 150 인 채다 (옛 100)
패널 2  Hz      152            147            140           그대로
```

**첫 자동 발행 회차 (cron 16:27 → 17:07)** — run 4340 이 시트에 붙었고(39 행, 되대조 통과) legacy 페어링 25 분을
포함해 40 분 걸렸다. **그런데 그림이 0/0 교체** — `map_file = config/websummary.map` 이 상대 경로라 cron 의
작업 디렉터리(홈)에서 못 찾았다. 손으로 돌릴 때는 저장소 안이라 드러나지 않았던 차이. `publish_google.py` 가
상대 경로를 params 파일 기준 저장소 루트로 풀도록 고쳤고(2ef0ebc, 시험 10/10), `cd /` 에서 다시 발행해 20/20 교체 확인.
**교훈 : cron 이 부르는 것은 `cd /` 에서 한 번 돌려 볼 것** (§11.126 의 `env -i` 시험과 같은 종류의 함정).

**제안 하나 더 — ch2 도 100 으로.** 패널 1 은 계수가 가장 큰 패널이고 09-01 HV 사고 뒤 이득이 내려온 상태라
(§11.167) 150 이 신호를 반쯤 자른다. 되돌리면 veto 총 계수가 옛 값(~750)을 넘고 두 패널이 더 산다.
나머지 죽은 일곱 패널(4·6·7·9·10·13·14)은 문턱 무관 — 현장 확인 그대로.
...
**★ 첫 판은 시트 주소가 깨져 나갔다 (04:5x).** `sed "s#표식#$URL#"` 로 넣었는데 URL 의 `&amp;` 속 `&` 가
sed 치환문에서 "일치한 문자열" 이라 `&amp;` 마다 표식 문구가 끼어들었다(그림 20 장은 멀쩡, 시트 iframe 만 400).
사용자가 이미 사이트에 붙인 뒤에 발견해 수정본을 다시 건넸다. **URL 을 sed 치환에 넣지 말 것 — python 으로.**
검증 : 블록 안 주소 21 개를 익명 curl 로 전부 200 확인(시트 표의 행 자체는 JS 렌더라 curl 로는 못 본다).

남은 것 : 사이트의 삽입 상자를 수정본으로 갈아 끼우기(사용자).
...
```
§11.205         ★ 타겟 뮤온 NPE 스펙트럼 veto 태그 유무·기간별 (MuonSpec.C, muspec/). 패널 AND 태깅 59 %(>3k)/72 %(>20k), 강한 veto 84/94 %.
                dst/4237 은 스키마 1 이라 재생성 중
...
§11.195         ★★★ "죽은 veto 패널 7 개" 는 오독 — 그 일곱이 450 m.w.e. 뮤온율(0.3~1.5 Hz, RENO far 13.1 Hz 환산)로 MIP 신호를 세는
                정상 카운터이고, '산' 여덟 패널은 뮤온율의 100 배로 문턱 바로 위 잡음을 센다(타겟 뮤온 동시 0.1 %). 문턱을 다시 올릴 것
