...
- **메일은 앱 비밀번호 하나만 남았다.** 서버·포트·수신자(책임자 1 / 전문가 6)는
  다 채웠고 `smtp_pass` 만 유효하지 않다 (§11.59). 그때까지 알람만 동작한다.
...
- **PC 스피커는 root 1회 조치가 필요하다. ★ `usermod` 로는 안 된다.**
  ...
  `docs/ALARM.md`. 안 해도 사운드카드로는 울린다.
...
생김새부터 다르다. 나머지(서버·포트·수신자)는 다 맞고 이것 하나만 남았다.
받은 주소에 오타가 있어(`stmp.gamil.com`) `smtp.gmail.com` 으로 바로잡았다.
