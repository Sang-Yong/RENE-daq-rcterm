...
/scratch   ★ 마운트 안 됨. 서버는 살아 있으나 10G 링크로 프레임이 한 장도 안 온다 (§11.133)
postrun    ★ 죽어 있다 — 'LOG 디렉터리 없음'. /scratch 가 없으면 못 돈다
dataflow   가동 중 (마운트 가드 넣은 판). 지금 run 4317 을 /Data_ssd -> /data 로 옮긴다
디스크     /Data_ssd 3.1T 여유 · /data 30T 여유
데이터     run 4317 완결 1440/1440/1440/1440.  run 4318 은 FADC 147 / PRD 145
...
| 무엇 | 왜 | 어디에 |
|---|---|---|
| 10G 링크 판별 — 저장소 서버 쪽에서 DAC 를 뽑고 이쪽 carrier 를 본다 | 케이블이냐 상대 설정이냐를 한 번에 가른다 | §11.133 |
| `mount /scratch` 뒤 postrun 재기동 | run 4318 의 꼬리 서브런 2개도 그때 처리 | §11.134 아래 |
| NM 프로파일 주소를 .71 로 되돌린다 | 재부팅하면 .75 로 떠서 끊긴다 | §11.132 |
