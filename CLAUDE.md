...
| **★ 실 하드웨어 24h 로테이션 (수정본)** | 2026-08-16 04:51:54 run 4288→4289. `ENDED run=004288` + `exit=code 0 (rotation)` + DB 완전 마감. **§11.1 버그 A 수정 확정** |
| **후처리 실시간 추적** | run 4288 PRD 1440개 전량 완료. run 4289 는 서브런 1371 대비 PRD 1368 — `--lag 3` 설정 그대로 따라붙음 |
...
| 2 | ~~수정본의 첫 로테이션 관찰~~ ✅ 2026-08-16 04:51 확인 완료 (§11.5) | — |
| 4 | **`/Data_ssd` 용량 — 가장 급한 것** | 실측 소비 **약 400 GB/일**(런 2개분 산출물). 여유 1.6 TB 이므로 **약 7일 뒤 가득 찬다.** 오래된 런의 Merged/PRD 를 `/scratch` 로 옮기거나 지우는 정책이 필요하다 |
