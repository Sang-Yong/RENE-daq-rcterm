...
void VetoHistoryPlot(const char* tsv="/scratch/RunSummary/psd/veto_history.tsv", const char* out="/scratch/RunSummary/psd/"){
  ...
  std::vector<int> pan; for(int q=0;q<15;q++){ double mx=0; for(auto&v:R){ if(v.size()>(size_t)(10+q) && v[0]>=4280 && v[10+q]>mx) mx=v[10+q]; } if(mx>=1.0) pan.push_back(q); }
  if(pan.empty()) pan={0,1,2,3,8,11};
  int col[8]={kRed+1,kBlue+1,kGreen+2,kMagenta+1,kOrange+7,kCyan+2,kViolet+1,kGray+2}; int mst[3]={20,21,22};
  ...
  auto page=[&](const char* name,const char* title,const char* yt,std::vector<std::pair<TGraph*,const char*>> gs, double lo){
    ...
    for(auto&p:gs){ p.first->SetMarkerStyle(mst[(k/8)%3]); p.first->SetMarkerColor(col[k%8]); p.first->SetLineColor(col[k%8]); mg->Add(p.first,"LP"); lg->AddEntry(p.first,p.second,"lp"); k++; }
  ...
  std::vector<std::pair<TGraph*,const char*>> P; for(size_t i=0;i<pan.size();i++) P.push_back({mk(10+pan[i],4280), Form("panel %d",pan[i])});
