...
//
//  무엇을 쓰나
//     <OutDir>/dst/DST_<NNNNNN>.root   T_Singles(+psd) + T_Sat + T_Muons + T_Info  (schema 2)
...
//  DST 스키마. 2 = T_Singles 에 psd 열, T_Sat 트리 추가 (배경 레시피 v2 스펙
//  docs/superpowers/specs/2026-09-08-background-recipes-v2-design.md §2).
//  single 의 개수·순서·pe 는 스키마 1 과 같다 -- legacy 패리티는 그대로다.
static const int kDstSchema = 2;
...
static bool DstUpToDate(const TString &path, int nSubNow) {
   ...
   Int_t nsub = -1, schema = 1;
   ti->SetBranchAddress("n_subrun", &nsub);
   ti->SetBranchAddress("schema",   &schema);
   ti->GetEntry(0);
   f->Close();
   //  서브런이 늘었거나 옛 스키마(psd·T_Sat 없음)면 다시 만든다
   return nsub == nSubNow && schema == kDstSchema;
}
...
static void BuildOne(int run, const TString &out, const TString &roots,
   ...
   std::vector<Float_t> psd;          // sing 과 같은 길이. 인덱스가 짝이다
   ...
   for (int sub : subs) {
      ...
      ReneSubrunStat st;
      size_t sBefore = sing.size(), mBefore = muons.size(), xBefore = sats.size();
      ...
      bool ok = ReneLoadCache(cpath, thr, vetoCutUs, sing, carry, st) &&
                ...
                ReneLoadCacheExtra(cpath, psd, sats) &&
                psd.size() == sing.size();
      if (!ok) {
         //  셋 중 하나라도 없으면(스키마 1 캐시) 파형에서 새로 만든다 (스키마 2 캐시로 덮어씀)
         carry = saved;
         sing.resize(sBefore); psd.resize(sBefore);
         muons.resize(mBefore); sats.resize(xBefore);
         TString prd = TString::Format("%sPRD/PRD_%s.%05d.root",
                          runDir.Data(), ReneRunStr(run).Data(), sub);
         st = ReneProcessSubrun(prd, sub, thr, vetoCutUs, carry, sing, &muons, &psd, &sats);
         if (!st.ok) { printf("  [WARN] run %d sub %d 읽기 실패\n", run, sub);
                       nBad++; continue; }
         ReneSaveCache(cpath, thr, vetoCutUs, sing, sBefore, carry, st,
                       &muons, mBefore, &psd, &sats, xBefore);
         nRead++;
      } else nFromCache++;
      liveS += st.liveSec;
   }
   w.Stop();
   if (psd.size() != sing.size()) {
      //  있을 수 없는 일이지만, 어긋난 짝으로 DST 를 쓰는 것보다 여기서 멈춘다
      printf("  [FAIL] run %d : psd %zu != singles %zu (짝이 어긋났다)\n",
             run, psd.size(), sing.size());
      return;
   }
   ...
   //  T_Sat : muon·after-muon 을 통과했으나 포화라 single 에서 빠진 사건.
   //  fast-n 사이드밴드(12 MeV 이상, 실측 93 % 포화)가 이것 없이는 성립하지
   //  않는다. pe 는 잘린 적분값(하한).
   TTree *tSat = new TTree("T_Sat", "saturated events past the muon cuts, time order");
   Int_t    x_sub = 0;
   Double_t x_t   = 0;
   Float_t  x_pe  = 0;
   tSat->Branch("sub_id", &x_sub);
   tSat->Branch("t_us",   &x_t);
   tSat->Branch("pe",     &x_pe);
   for (const auto &xc : sats) {
      x_sub = xc.sub; x_t = xc.t_us; x_pe = xc.pe;
      tSat->Fill();
   }
   ...
   printf("  [DST]  run %d : singles %zu  sat %zu  muons %zu  live %.0f s  "
          "(캐시 %d / 새로 %d / 실패 %d)  [%.1f s]\n",
