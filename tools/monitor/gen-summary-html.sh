...
#
# 열 15개, run_summary.tsv/pair_summary.tsv/metrics_summary.tsv 의 1-based
# 열 번호는 각 파일의 WriteTsv() 가 정본이다(BuildRunSummary.C/
# BuildPairSummary.C/BuildMetrics.C):
   ...
   END {
      ...
      for (r in have) {
         ...
         printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", \
                r, start, lw, total, t1[r], t2[r], t3[r], \
