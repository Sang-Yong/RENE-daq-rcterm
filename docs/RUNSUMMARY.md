...
```
   ...
   4d daily.sh              daily_summary.tsv + 32..52_*.png   날짜 기준 라이브타임·후보·rate + 전체 사건 prompt/delayed 스펙트럼(배경 빼기 전/후)
                            + 배경 성분별 스펙트럼(41~44) · 신호창 분해(45~48) · prompt PSD(49/50) · 샤워 뒤 Δt 와 Li/He 적합(51/52). 런별 표와 별개
                            손잡이 daily_psd_nsig · fn_norm_mode · fn_norm_lo_mev (config/monitorcuts.params)
