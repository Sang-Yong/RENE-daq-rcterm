# 세션 기록 아카이브 — 2026-08-13 ~ 2026-08-25

`CLAUDE.md` 는 매 세션 통째로 읽히므로 무한정 키울 수 없다. 그래서 **오래된 세션
기록만 이 파일로 옮겼다.** 지운 것은 없다 — 절 번호(§11.x)도 그대로다.

**언제 이 파일을 여나**
* `CLAUDE.md` 가 §11.100 이하의 절 번호를 가리킬 때
* 옛 장애의 경위나 그때의 실측값을 확인해야 할 때

**평소에는 열 필요가 없다.** 지금 쓰는 규칙·설계·주의점은 전부 `CLAUDE.md`
§0~§10 에 있고, 최근 작업은 `CLAUDE.md` §11 에 있다.

여기 담긴 것 (새 것부터) : 로그 디렉터리 재구성과 NFS 포트 차단(08-25) ·
PRD 결손 복구 연작(08-22~24) · badrun 격리 체계(08-21) · 알람/자동복구와 FADC
보드 장애(08-20) · 체크섬 이동 규칙(08-19) · 모니터링 3단계(08-18) ·
데이터 흐름 설계(08-17) · 로테이션 버그 3종 수정(08-15~16) · 최초 검증(08-13)
