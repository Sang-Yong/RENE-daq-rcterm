...
이 PC 에는 쓰는 랜카드가 둘이고, 목적지가 다르다.

| 인터페이스 | 속도 | 무엇이 지나가나 | 실측 처리량 |
|---|---|---|---|
| `enp3s0f1` (10.0.0.11) | **10 Gb** | `/scratch` NFS | **읽기 838 · 쓰기 534 MB/s** |
| `enp0s31f6` (203.230.111.71) | 1000 Mb | 경희대 백업 | **15.7 MB/s** (링크의 13%) |

**★ 2026-08-26 에 스토리지 링크를 100 Mb 에서 10 Gb 로 바꿨다.** Intel X710-2
SFP+ 를 새로 꽂고 DAC 로 직결했다. 그 전에는 `enp1s0`(Aquantia AQC113CS)가
**100 Mb 로 협상되어 7.7 MB/s** 였고, 이 문서의 이전 판과 `CLAUDE.md` §5.8 의
여러 판단이 그 값을 전제로 하고 있었다. 설정 절차와 실측은 `CLAUDE.md` §11.115.

| 측정 | 옛 100 Mb | 새 10G |
|---|---|---|
| `/scratch` 읽기 | — | **838 MB/s** |
| `/scratch` 쓰기 (버퍼드) | 7.7 MB/s | **534 MB/s** |
| RTT | 10.6 ms | **0.31 ms** |

**★ `oflag=direct` 로 NFS 쓰기를 재지 말 것.** 1 MB 마다 서버 커밋을 기다리는
왕복 지연 측정이라 62.8 MB/s 로 나온다. rsync·ROOT 쓰기·dataflow 는 전부
버퍼드이므로 실제 값은 534 MB/s 다.

여전히 유효한 것 :

- **2단계와 3단계는 서로 대역을 뺏지 않는다.** 경희대로 가는 길과 `/scratch`
  로 가는 길이 다른 랜카드라, 동시에 돌려도 된다.
- 경희대 쪽 15.7 MB/s 는 **바깥 경로의 한계다** — 2026-08-26 에 확정했다(아래).
  **이제 여기가 가장 느린 구간이다.**
- 수집과 후처리는 계속 로컬 NVMe(`/Data_ssd`)에서 끝낸다. 링크가 빨라졌다고
  NFS 위에서 하는 것이 낫다는 뜻은 아니다 — `/Data_ssd` 쓰기가 2.0 GB/s 다.

**★ 아직 실측하지 않은 것** — 링크가 100배가 됐으므로 3단계 소요 시간,
체크섬 대조 비용(`CLAUDE.md` §11.63), 후처리의 `/scratch` 대 `/Data_ssd`
차이(§11.32)를 **전부 다시 재야 한다.** 아래 표의 3단계 값은 산술이다.

### 경희대 백업 경로는 왜 느린가 — 10 Gb 로 올려도 소용없다

2026-08-26 실측. **병목은 이 PC 의 랜카드가 아니라 바깥 경로다.**
...
**스트림을 5배로 늘려도 10% 밖에 안 는다** — 경로가 약 145 Mbps 에서 막혀 있다.
`enp0s31f6` 은 1 Gb 인데 **13%만 쓰인다.** TCP 상태를 보면 재전송 0.091% 로
혼잡창이 69 세그먼트(~100 KB)에 눌려 있고, 병렬로도 안 뚫리므로 혼잡이 아니라
**어딘가의 속도 제한**이다. 한산한 새벽에 재도 같은 값이다.

| 무엇을 바꾸나 | 백업 1회 (221 GB) | 얻는 것 |
|---|---|---|
| 지금 | **3.9 시간** | — |
| 랜카드만 10 Gb 로 | **3.9 시간** | **0. 아무것도 없다** |
| 경로가 1 Gb 를 내주면 | **약 33 분** | **7배** |
| 경로까지 10 Gb | 약 6 분 | 그 위로 5배 더 |

**이미 꽂혀 있는 1 Gb 카드가 제 몫을 하게만 해도 7배다** — 하드웨어를 사지 않고
얻는다. 그리고 1 Gb 면 충분하다 : 24시간 런 하나가 33분이면 **하루의 2.3%** 다.
자세한 것과 사이트에 물어볼 것은 `CLAUDE.md` §11.121.
...
| 작업 | 옮기는 양 | 링크 | 걸리는 시간 |
|---|---|---|---|
| 2단계 백업 (RAW + PRD) | 221 GB | 15.7 MB/s | **약 3.9 h** (실측) |
| 3단계 `/scratch` (전부) | 334 GB | 534 MB/s | 약 **11 분** (산술, 미실측) |
| ~~3단계 (`--drop-merged`)~~ | | | 켤 이유가 없어졌다 |

**이제 3단계는 사실상 공짜다.** 옛 100 Mb 에서는 전부 옮기는 데 12.1 시간이라
하루의 절반을 썼고, 그래서 `--drop-merged` 로 Merged 를 버리는 선택지를 두었다.
**링크가 10 Gb 가 된 지금은 그럴 이유가 없다.**

**★ 3단계 값은 아직 산술이다.** 실제로 도는 것을 보고 채울 것. 체크섬 대조
비용도 마찬가지다 — 옛 링크에서는 대조가 전송보다 비쌌는데(`CLAUDE.md` §11.63),
그 전제가 통째로 바뀌었다. 병목은 이제 링크가 아니라 서버 스토리지다.
