...
`--lag N`(기본 2) 만큼 더 물러나 처리한다. NFS 서버 시계가 로컬보다
약 28초 앞서 있는 것을 관측했으므로 여유가 필요하다.

```
subrun 842  기록 중        <- 절대 건드리지 않는다
subrun 841  완료           <- lag 여유
subrun 840  ─────> 처리 상한
```
...
# 수집을 뒤따라가며 계속 처리
scripts/postrun.sh --follow --jobs 3
